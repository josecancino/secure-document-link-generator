...
export class AppService implements OnModuleInit {
  ...
  generateLink(documentName: string): { token: string; link: string } {
    if (!documentName || documentName.trim().length < 3) {
      throw new BadRequestException('Document name must be at least 3 characters long.');
    }

    if (documentName.length > 50) {
      throw new BadRequestException('Document name is too long (max 50 characters).');
    }
    ...
    this.logger.log(`[DEBUG_TRACE]: Generated link for document: "${documentName}" (Token: ${token.substring(0, 8)}...)`);
  ...
  redeemLink(token: string): { documentName: string } {
    const query = this.db.query(`
      ...
      AND createdAt >= datetime('now', '-15 minutes')
    `);
    ...
    this.logger.log(`[DEBUG_TRACE]: Link redeemed for document: "${link.documentName}" (Token: ${token.substring(0, 8)}...)`);
